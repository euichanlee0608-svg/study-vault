...
<h4>5. 회로이론의 전제와 실물 — 집중소자·접지·실습 도구 <span class="en">(Lumped Elements, Grounds, Lab Practice)</span><span class="tagc">개념 확인</span></h4>
<div class="whyq"><b>❓ 문제 상황</b> — 모든 전기 현상은 맥스웰 방정식이다. 그런데 우리는 저항·축전기·인덕터와 KCL·KVL만으로 푼다. 이 "지름길"은 언제 허락되는가? 그리고 회로도의 접지 기호는 실물의 무엇인가?</div>
<p><b>집중소자 가정</b><span class="en">(lumped-element assumption)</span> — 전기 에너지는 C에, 자기 에너지는 L에, 소산은 R에 <b>몰려 있다</b>고 본다.
성립 조건은 회로 치수 \(d\)가 신호 파장 \(\lambda=c/f\)보다 충분히 작은 것(흔한 경험칙 \(d/\lambda<0.1\)). 이를 넘으면 같은 도선 위에서도 위치마다 전압이 달라지는 분포정수 영역이다.</p>
<p><b>접지 3종</b> — ① 신호 접지<span class="en">(signal ground)</span>: 전위 0으로 <b>정한</b> 도체, 절점해석의 원점. 땅과 연결됐다는 뜻이 아니다.
② 대지 접지<span class="en">(earth ground)</span>: 땅에 박은 금속. 안전·잡음 때문에 신호 접지를 여기에 잇기도 한다.
③ 섀시 접지<span class="en">(chassis ground)</span>: 회로를 덮는 금속 케이스, 보통 안전을 위해 대지에 연결.
교재 기호는 셋을 구분하지 않으니 문맥으로 읽는다(대개 신호 접지). 서로 다른 접지를 여러 경로로 이으면 <b>그라운드 루프</b>가 생겨 잡음원이 된다.
자동차의 <b>차체 귀환</b><span class="en">(earth-return)</span>은 금속 차체 자체를 공통 귀환 도체로 쓰는 예다.</p>
<p><b>실습 도구</b> — 브레드보드는 시험용(영구 회로는 PCB), 제작 흐름은 회로도 → 레이아웃 → 거버 → PCB → 납땜 → 시험. 계측기에는 한계가 있다(멀티미터 AC 측정 대역 등) — 실험 3회의 서술형과 이어진다.</p>
<p class="intuit">💡 <b>직관</b> — 공유 접지선도 저항이 0이 아니다. 다른 회로의 큰 전류가 흐르면 \(\Delta V=I R_g\)만큼 "접지가 흔들린다". 접지 = 이상적인 0 V라는 가정도 집중소자 가정처럼 조건부다.</p>
<div class="exambox"><span class="xt">시험 연결</span>[개념] 집중소자 성립 조건(u1-l1-11), 접지 3종(u1-l1-12), 그라운드 루프(u1-l1-13), 차체 귀환 경로(u1-l1-14), 브레드보드·PCB 공정(u1-l1-15/16), 멀티미터 대역(u1-l1-17). [계산] d/λ 판정(u1-l2-16), 공유 접지 오차(u1-l3-15), 차체 귀환 전압강하(u1-l3-16).</div>""",
