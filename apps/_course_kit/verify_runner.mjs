...
const KIT = path.dirname(url.fileURLToPath(import.meta.url));
const DIR = path.resolve(process.argv[2]);           // <app>/pipeline
const N = parseInt(process.argv[3] || '50', 10);

const files = [path.join(KIT, 'helpers.js'),
  ...fs.readdirSync(path.join(DIR, 'problems')).filter(f => /^u\d+\.js$/.test(f)).sort()
    .map(f => path.join(DIR, 'problems', f))];
  ...
  for (const prob of unit.problems) {
    ...
    if (prob.type === 'mc') {
      ...
      for (let i = 0; i < nRuns; i++) {
        ...
        } catch (e) { rec.samples.push({ p, err: String(e.message || e) }); }
      }
    }
