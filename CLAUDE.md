...
- **개념 과목(course-bio)은 SPEC §10** — 전 문항 `ref`, 정답은 블라인드 풀이 에이전트와 대조(`pipeline/blind/`), 불일치는
  키를 맞추지 말고 자료로 판정해 `blind_review.json` 에 근거. `_course_kit/` 게이트·러너·엔진을 고치면 기존 과목 4개 게이트도 다시 돌린다.
...
- **떠 있는 필기장은 `shared/notepad.js`** — 터치펜으로 쓰는 손글씨 캔버스(이동·크기조절·켜고끄기).
  타이핑이 아니다(2026-09-13에 textarea → canvas 로 교체). 획을 좌표 배열로 들고 있다가 다시 그리므로
  **창 크기를 바꿔도 글씨가 뭉개지지 않는다** — 비트맵을 늘리지 말 것.
  **일부러 저장하지 않는다**: 새로고침하면 지워지는 게 사양이라 localStorage 를 쓰지 않는다
  (2026-09-12, 저장·과목별 분리 버전을 만들었다가 "그냥 단순한 거"로 되돌림).
  펜을 한 번 보면 손가락 입력을 무시한다(손바닥 오터치 방지) — 펜 없이 손가락만 쓰는 기기는 그대로 된다.
