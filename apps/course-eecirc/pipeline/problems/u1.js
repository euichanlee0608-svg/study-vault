...
SV_BANK.push({
  ...
  /* L1 보강 — Orientation-2026 강조 개념(집중소자·접지·실습 도구) + 기출 계측기 한계 (2026-09-14) */
  { id:'u1-l1-11', level:1, type:'tf', tags:['집중소자'], src:'강의자료 대조',
    statement:'회로이론(집중소자 모델, lumped-element model)은 맥스웰 방정식을 대신하는 근사로, 회로의 물리적 치수가 신호 파장보다 충분히 작을 때에만 성립한다.',
    answer:true, expl:'에너지를 C(전기장)·L(자기장)·R(소산)에 "몰아 둔다"는 가정이다. 치수가 파장에 가까워지면 같은 도선 위에서도 위치마다 전압·전류가 달라져(분포정수) KCL·KVL을 그대로 쓸 수 없다 — 고주파 기판·안테나가 그 영역.' },
  { id:'u1-l1-12', level:1, type:'mc', tags:['접지 구분'], src:'강의자료 대조',
    statement:'접지(ground) 세 종류에 대한 설명으로 옳은 것은?',
    choices:['신호 접지는 전위의 기준으로 정한 도체일 뿐, 반드시 땅(대지)에 연결됐다는 뜻은 아니다','섀시 접지는 회로의 전위 기준 도체를 뜻하며 금속 케이스와는 무관하다','대지 접지와 신호 접지는 항상 같은 점이므로 구분할 필요가 없다','교재(Rizzoni) 회로도의 접지 기호는 세 종류를 엄격히 구분해 그린다'],
    answer:0, expl:'신호 접지 = 계산의 원점(절점전압 0), 대지 접지 = 땅에 박은 금속, 섀시 접지 = 금속 케이스(안전 때문에 보통 대지에 연결). 교재 기호는 이를 구분하지 않으니 문맥으로 읽는다 — 대개 신호 접지다.' },
  { id:'u1-l1-13', level:1, type:'tf', tags:['그라운드 루프'], src:'강의자료 대조',
    statement:'장비들의 접지를 가능한 한 여러 경로로 서로 연결할수록 잡음이 줄어든다.',
    answer:false, expl:'반대다. 접지 경로가 여럿이면 폐루프(그라운드 루프)가 생기고, 루프를 관통하는 자기장 변화나 공유 도선에 흐르는 다른 전류가 접지 사이 전압차를 만들어 잡음이 된다. 서로 다른 접지는 따져 보고 잇는다.' },
  { id:'u1-l1-14', level:1, type:'mc', tags:['차체 귀환'], src:'강의자료 대조',
    statement:'음극 접지 차체 귀환(negative earth-return) 방식 자동차에서 헤드램프 전류의 경로로 옳은 것은?',
    choices:['배터리 (+) → 전선 → 램프 → 금속 차체 → 배터리 (−)','배터리 (−) → 전선 → 램프 → 금속 차체 → 배터리 (+)','배터리 (+) → 전선 → 램프 → 전선 → 배터리 (+)','배터리 (+) → 금속 차체 → 램프 → 땅(대지) → 배터리 (−)'],
    answer:0, expl:'공급은 전선 한 가닥, 귀환은 금속 차체다. 차체가 굵은 케이블로 배터리 (−)에 묶여 있어 차체 전체가 공통 귀환 도체(=접지)가 된다. 여기서 earth는 땅이 아니라 이 공통 도체를 뜻한다 — 차는 타이어로 땅과 떨어져 있다.' },
  { id:'u1-l1-15', level:1, type:'tf', tags:['실습 도구'], src:'강의자료 대조',
    statement:'브레드보드(breadboard)는 납땜 없이 부품을 꽂는 방식이라 완성 제품의 영구 회로로 쓰기에 가장 적합하다.',
    answer:false, expl:'브레드보드는 아이디어를 빨리 시험하고 문제를 찾는 용도다. 접촉이 헐거워지고 줄 사이 기생 용량·접촉 저항이 있어 영구 회로는 PCB에 납땜한다. 실험값이 이론과 어긋날 때 먼저 의심할 곳이기도 하다.' },
  { id:'u1-l1-16', level:1, type:'mc', tags:['실습 도구','PCB 공정'], src:'강의자료 대조',
    statement:'회로를 실제 기판(PCB)으로 만드는 일반적인 순서로 옳은 것은?',
    choices:['회로도(schematic) → 레이아웃(layout) → 거버 파일(Gerber) → PCB 제작 → 납땜 → 시험','레이아웃 → 회로도 → PCB 제작 → 거버 파일 → 시험 → 납땜','회로도 → PCB 제작 → 레이아웃 → 납땜 → 거버 파일 → 시험','거버 파일 → 회로도 → 납땜 → 레이아웃 → PCB 제작 → 시험'],
    answer:0, expl:'논리 연결(회로도) → 물리 배치(레이아웃) → 제조사에 넘기는 표준 도면(거버) → 기판 제작 → 부품 실장(납땜) → 시험. 거버는 레이아웃에서 뽑아내는 출력물이라 레이아웃보다 앞설 수 없다.' },
  { id:'u1-l1-17', level:1, type:'tf', tags:['계측'], src:'기출 유형',
    statement:'일반 디지털 멀티미터의 교류(AC) 전압 측정은 정해진 주파수 대역 안에서만 정확하므로, 대역을 넘는 고주파 신호의 크기는 오실로스코프로 확인해야 한다.',
    answer:true, expl:'기출 1번 계열의 "계측기 한계" 진술. 멀티미터는 저주파 정현파를 기준으로 만든 기종이 많아 고주파·비정현파에서는 읽음값이 틀어진다. 허용 대역은 기종 사양서에 있으니 측정 전에 확인한다.' },
